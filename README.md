...
- La interpolación ráster utiliza todos los puntos fijos en su orden y sigue la geometría real del recorrido para formar un corredor continuo entre ellos. Combina frecuencia, estancia y proximidad, mantiene el radio de 5 m y agrega un borde exterior sin concentración. La paleta es verde claro `#ADEEC5` (sin concentración), amarillo `#FFFB7D` (baja), naranja `#FFB23C` (alta) y rojo oscuro `#B73225` (muy alta).
